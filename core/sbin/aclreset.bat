...
:loop
    ...
    if %_verbose% geq 0 echo [reset] "%~1"
    rem yes | cacls %1 %_caclsopt% %_outf%
    setacl -on "%~dpnx1" -ot file -rec cont_obj %a_owner% %_outf%
    if errorlevel 1 echo   error reset owner
    setacl -on "%~dpnx1" -ot file -rec cont_obj %a_reset% %a_clear% %_outf%
