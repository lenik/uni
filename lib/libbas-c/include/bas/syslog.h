#ifndef __BAS_SYSLOG_H
#define __BAS_SYSLOG_H
...
#define LOG_IF_LEVEL(level, ...) \
    if (level > LOG_LEVEL); \
    else _syslog_x(LOG_IDENT, LOG_OPTION, level, __VA_ARGS__)

#define LOG_PERROR_IF_LEVEL(level, ...) \
    if (level > LOG_LEVEL); \
    else _syslog_x_perror(LOG_IDENT, LOG_OPTION, level, __VA_ARGS__)
