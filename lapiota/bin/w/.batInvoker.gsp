@echo off
    <%
        String GENERATOR = "batInvoker";
        String TEMPLATE = ".batInvoker.gsp";
        assert type != null;
        assert name != null;
        if (target == null) target = "%~n0";
        ...
        String typeExts = "";
        if ("perl".equals(type)) {
            typeOpts = "-w -Mcmt::mess";
            typeExts = ".pl .p .pc .pld";
            winVer = true;
        } else if ("autoit3") {
            typeExts = ".au3";
        }
        String typew = type + (winVer ? "w" : "");
        String TYPEW = TYPE + (winVer ? "W" : "");
    %>
...
:startc
    ...
    if %_verbose% geq 1 (
        echo "%<%= TYPE%>%" %_<%= type%>opts% "%_nam%" %_rest%
    )
    "%<%= TYPE%>%" %_<%= type%>opts% "%_nam%" %_rest%
    exit /b

:startw
    if "%<%= TYPEW%>%"=="" set <%= TYPEW%>=<%= typew%>
    if %_verbose% geq 1 (
        "%<%= TYPEW%>%" %_<%= type%>opts% "%_nam%" %_rest%
    )
    "%<%= TYPEW%>%" %_<%= type%>opts% "%_nam%" %_rest%
    exit /b
...
:init
    ...
    set      _nam=<%= target %>
    set      _ext=
    set _<%= type%>opts=<%= typeOpts%>

:find_target
    for %%x in ("" <%= typeExts%>) do (
        set _ext=%%~x
        if exist "%_nam%%%~x" (
            if not exist "%_nam%%%~x\*" (
                set _dir=
...
:help
    ...
    echo.
    echo Options:
    echo    -Pw,--pb-win        start with <%= typew%>.exe
