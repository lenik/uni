/** This overlay patch changed the visibility of getWelcomeFile to protected */
 ...
 * these can be set either on the servlet itself or as ServletContext initParameters with a prefix
 ...
 *
 */
public class DefaultServlet_nonprivate_patch
        extends HttpServlet
    ...
    protected boolean _welcomeServlets = false;
    protected boolean _welcomeExactServlets = false;
    ...
    boolean _useFileMappedBuffer = false;
     ...
     * In the case where the DefaultServlet is deployed on the HttpService it is likely that this
     * method needs to be overwritten to unwrap the ServletContext facade until we reach the
     * original jetty's ContextHandler.
     *
    ...
    private boolean getInitBoolean(String name, boolean dft) {
        ...
        if (value == null || value.length() == 0)
            return dft;
        return (value.startsWith("t") || value.startsWith("T") || value.startsWith("y") || value.startsWith("Y") || value
                .startsWith("1"));
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (LOG.isDebugEnabled())
                LOG.debug("uri=" + request.getRequestURI() + " resource=" + resource
            ...
            if (resource == null || !resource.exists()) {
                ...
                if (endsWithSlash && _contextHandler.isAliases() && pathInContext.length() > 1) {
                    ...
                    if (q != null && q.length() != 0)
                        pathInContext += "?" + q;
                    response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(_servletContext.getContextPath(),
                            pathInContext)));
                } else {
                    // ensure we have content
                    if (content == null)
                        content = new HttpContent.ResourceAsHttpContent(resource,
                                _mimeTypes.getMimeByExtension(resource.toString()), response.getBufferSize());
                ...
                // else look for a welcome file
                    ...
                    if (_redirectWelcome) {
                        ...
                        String q = request.getQueryString();
                        if (q != null && q.length() != 0)
                            response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(
                                    _servletContext.getContextPath(), welcome)
                                    + "?" + q));
                        else
                            response.sendRedirect(response.encodeRedirectURL(URIUtil.addPaths(
                                    _servletContext.getContextPath(), welcome)));
    ...
    /* ------------------------------------------------------------ */
    /**
     * Finds a matching welcome file for the supplied {@link Resource}. This will be the first entry
     * in the list of configured {@link #_welcomes welcome files} that existing within the directory
     * referenced by the <code>Resource</code>. If the resource is not a directory, or no matching
     * file is found, then it may look for a valid servlet mapping. If there is none, then
     * <code>null</code> is returned. The list of welcome files is read from the
     * {@link ContextHandler} for this servlet, or <code>"index.jsp" , "index.html"</code> if that
     * is <code>null</code>.
     *
     * @param resource
    ...
    protected boolean passConditionalHeaders(HttpServletRequest request, HttpServletResponse response,
            Resource resource, HttpContent content)
            throws IOException {
        try {
    ...
    protected void sendData(HttpServletRequest request, HttpServletResponse response, boolean include,
            Resource resource, HttpContent content, Enumeration<?> reqRanges)
            throws IOException {
        boolean direct;
        long content_length;
        if (content == null) {
            ...
            direct = connector instanceof NIOConnector && ((NIOConnector) connector).getUseDirectBuffers()
        ...
        try {
            ...
            written = out instanceof HttpOutput ? ((HttpOutput) out).isWritten() : AbstractHttpConnection
        ...
        if (reqRanges == null || !reqRanges.hasMoreElements() || content_length < 0) {
            ...
            for (int i = 0; i < ranges.size(); i++) {
                ...
                length += ((i > 0) ? 2 : 0) + 2 + multi.getBoundary().length() + 2 + HttpHeaders.CONTENT_TYPE.length()
                        + 2 + mimetype.length() + 2 + HttpHeaders.CONTENT_RANGE.length() + 2 + header[i].length() + 2
                        + 2 + (ibr.getLast(content_length) - ibr.getFirst(content_length)) + 1;
            }
