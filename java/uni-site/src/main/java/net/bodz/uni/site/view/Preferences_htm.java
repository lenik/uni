...
public class Preferences_htm
        extends AbstractHttpViewBuilder<Preferences> {
    ...
    @Override
    public void preview(IHttpViewContext ctx, IUiRef<Preferences> ref, IOptions options) {
        IHtmlHeadData metaData = ctx.getHeadData();
        metaData.setTitle("User Preferences");
    }

    @Override
    public IHtmlTag buildHtmlView(IHttpViewContext ctx, IHtmlTag out, IUiRef<Preferences> ref, IOptions options)
