...
public class Base45Encoder {
     ...
     * an array definition. But I was unsure of the licensing issues with posting it on the
     * internet, so I did it this way.
     */
    static {
        ...
        try {
            SOURCE_ALPHANUMERIC_TABLE = com.google.zxing.qrcode.encoder.Encoder.class
                    .getDeclaredField("ALPHANUMERIC_TABLE");
    ...
    static {
        ...
        for (int x = 0; x < len; x++) {
            ...
            if (base45DigitValue > -1) {
                ...
                //
                // i.e. to produce a base45 digit with the numeric value in base45DigitValue, we
                // need
    ...
    /*
     * The storage capacity of one digit in the number system; i.e. the maximum possible number of
     * distinct values which can be stored in 1 logical digit
     */
    public static final int QR_PAYLOAD_NUMERIC_BASE = NUM_DISTINCT_ALPHANUM_VALUES;

    /*
     * We can't use all 8 bytes, because the Long is signed, and the conversion math requires
     * consistently positive values. If we populated all 8 bytes, then the last byte has the
     * potential to contaminate the sign bit, and break the conversion math. So, we only use the
     * lower 7 bytes, and avoid this problem.
     */
    public static final int LONG_USABLE_BYTES = Long.BYTES - 1;

    // The following mapping was determined by brute-forcing -1 Long (all bits 1), and
    // compressing to base45 until it hit zero.
    ...
    public static String encodeToBase45QrPayload(byte[] inputData)
            throws IOException {
        return encodeToBase45QrPayload(inputData, 0, inputData.length);
    }

    public static String encodeToBase45QrPayload(byte[] inputData, int off, int len)
            throws IOException {
        return encodeToBase45QrPayload(new ByteArrayInputStream(inputData, off, len));
    }

    public static String encodeToBase45QrPayload(final InputStream in)
            throws IOException {
        ...
        // Process all input data in chunks of size LONG.BYTES, this allows for economies of
        // scale
        ...
        // left over; i.e. the slack space between where the multiples of
        // QR_PAYLOAD_NUMERIC_BASE
        ...
        while (in.available() > 0) {
            ...
            while (numB45DigitsProcessed < numBase45Digits) {
                ...
                // Write Digit
                outputChunkBuffer.append(alphaNumReverseIndex[digit]);
             ...
             * flipping them back on the decode side, but just to be safe, I'm always keeping them
             * in the proper order.
             */
    ...
    public static byte[] decodeBase45QrPayload(final String inputStr)
            throws IOException {
        // Prep for InputStream
        final byte[] buf = inputStr.getBytes();// Use the default encoding (the same encoding
                                               // that the 'char' primitive uses)

        return decodeBase45QrPayload(new ByteArrayInputStream(buf));
    }

    @SuppressWarnings("unused")
    public static byte[] decodeBase45QrPayload(final InputStream in)
            throws IOException {
        // Init conversion state vars
        ...
        while (in.available() > 0) {
            ...
            while (numBytesProcessed < numBytes) {
                ...
                //
                // Again, we need to invert the order of the bytes, so as we chunk them off,
                // push
